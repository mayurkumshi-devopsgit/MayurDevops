...
do
  ...
  # Install the httpd package                               // on remote machine 
  ssh ${SERVER} sudo yum install -y httpd 

  # Create an index.html file.
  ssh ${SERVER} 'echo "${HOSTNAME}" | sudo tee /var/www/html/index.html >/dev/null'         // index.html is home page for apache server . 

  # Start httpd
  ssh ${SERVER} sudo systemctl start httpd             //nexttime machine it will automatically start.

  # Enable httpd
  ssh ${SERVER} sudo systemctl enable httpd                      

  # Test that the web server is accessible.                       // port 80 enable 
  curl http://${SERVER}
